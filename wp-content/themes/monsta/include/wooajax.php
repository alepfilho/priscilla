...
add_action('wp_head','monsta_woo_ajaxurl');
function monsta_woo_ajaxurl() {
?> 
<?php
...
function product_quickview() {
	...
	<div class="woocommerce product">
		<div class="product-images">
			...
			<div class="main-image images"><img src="<?php echo esc_url($image_link);?>" /></div>
			<?php
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($monsta_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<div class="social-sharing"><?php monsta_product_sharing(); ?></div>
		</div>
	</div>
	<?php
	die();
...
function get_cartinfo() {
	...
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'monsta'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	?>
	<h3><?php esc_html_e('Product is added to cart', 'monsta');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo esc_url(get_permalink( wc_get_page_id( 'cart' ) ));?>"><?php esc_html_e('View Cart', 'monsta');?></a>
	</div>
	<?php
	die();
}
