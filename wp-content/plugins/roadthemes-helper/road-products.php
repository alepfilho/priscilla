...
class RoadProducts {
	...
	public static function sale_products_r( $atts ) { 
		...
		return '<div class="sale_product '.$slider.' '.$enable_time.' '.$style.'">'.$shortcode->get_content().'</div>';
	}
	...
	public static function bestselling_products_r( $atts ) {
		...
		$shortcode = new WC_Shortcode_Products( $atts, 'best_selling_products' );
			...
			return '<div class="best_selling_products '.$slider.' '.$style.'">'.$shortcode->get_content().'</div>' ;
	...
	public static function recent_products_r( $atts ) {
		...
		return '<div class="recent_products '.$slider.' '.$style.'">'.$shortcode->get_content().'</div>';
	}
	...
	public static function featured_products_r( $atts ) { 
		...
		return '<div class="featured_products '.$slider.' '.$style.'">'.$shortcode->get_content().'</div>';
	}
	...
	public static function list_products_r( $atts ) {
		...
		return '<div class="list_product '.$slider.' '.$style.'">'.$shortcode->get_content().'</div>';
	}
	...
	public static function product_category_r($atts) { 
		...
		return '<div class="category_product '.$slider.' '.$style.'">'.$shortcode->get_content().'</div>';
	...
	public static function product_categories_r( $atts ) { 
		...
		return '<div class="categories_product '.$slider.' '.$style.' woocommerce columns-' . $columns . '">' . ob_get_clean() . '</div>';
	}
	...
	public function road_vc_shortcodes() {
		...
		$order_by_values = array(
			...
			__( 'Date', 'monsta' ) => 'date',
			__( 'ID', 'monsta' ) => 'ID',
			__( 'Author', 'monsta' ) => 'author',
			__( 'Title', 'monsta' ) => 'title',
			__( 'Modified', 'monsta' ) => 'modified',
			__( 'Random', 'monsta' ) => 'rand',
			__( 'Comment count', 'monsta' ) => 'comment_count',
			__( 'Menu order', 'monsta' ) => 'menu_order',
		);

		$order_way_values = array(
			'',
			__( 'Descending', 'monsta' ) => 'DESC',
			__( 'Ascending', 'monsta' ) => 'ASC',
		); 
		$product_style_values = array(
			'Default', 
			__( 'Product Styles 2', 'monsta' ) => 'style_2', 
		); 
		 
		 
	//Sale products Roadthemes
	vc_map( array(
		'name' => __( 'RT Sale products', 'monsta' ),
		'base' => 'sale_products_r',
		'icon' => 'icon-wpb-woocommerce',
		'category' => __( 'Theme', 'monsta' ),
		'description' => __( 'List all products on sale', 'monsta' ),
		'params' => array_merge(
			array(
				array(
					'type' => 'checkbox',
					'heading' => __( 'Enable Time Sale', 'monsta' ),
				...
				array(
					'type' => 'textfield',
					'heading' => esc_html__( 'Per page', 'monsta' ),
					'value' => 12,
					'save_always' => true,
					'param_name' => 'per_page',
					'description' => esc_html__( 'How much items per page to show', 'monsta' ),
				),
					array(
					'type' => 'textfield',
					'heading' => __( 'Columns', 'monsta' ),
					'value' => 4,
					'save_always' => true,
					'param_name' => 'columns',
					'description' => __( 'How much columns grid', 'monsta' ),
				),
			 
					array(
					'type' => 'dropdown',
					'heading' => __( 'Product Styles', 'monsta' ),
					'param_name' => 'product_styles',
					'value' => $product_style_values,
					'save_always' => true,
					'description' => __( 'Select Styles for product', 'monsta' ),
				),
				array(
					'type' => 'dropdown',
					'heading' => __( 'Order by', 'monsta' ),
					'param_name' => 'orderby',
					'value' => $order_by_values,
					'save_always' => true,
					'description' => sprintf( __( 'Select how to sort retrieved products. More at %s.', 'monsta' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				),
				array(
					'type' => 'dropdown',
					'heading' => __( 'Sort order', 'monsta' ),
					'param_name' => 'order',
					'value' => $order_way_values,
					'save_always' => true,
					'description' => sprintf( __( 'Designates the ascending or descending order. More at %s.', 'monsta' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				), 
	...
	vc_map( array(
		'name' => __( 'RT Featured products', 'monsta' ),
		'base' => 'featured_products_r',
		'icon' => 'icon-wpb-woocommerce',
		'category' => __( 'Theme', 'monsta' ),
		'description' => __( 'List all products on Featured', 'monsta' ),
		'params' => array_merge(
		array(
			array(
				'type' => 'textfield',
	...
	vc_map( array(
	'name' => __( 'RT Recent products', 'monsta' ),
	'base' => 'recent_products_r',
	'icon' => 'icon-wpb-woocommerce',
	'category' => __( 'Theme', 'monsta' ),
	'description' => __( 'List all products on Recent', 'monsta' ),
	'params' => array_merge( 
		array(
			array(
				'type' => 'textfield',
	...
	vc_map( array(
	'name' => __( 'RT Bestselling products', 'monsta' ),
	...
	//products Roadthemes
	vc_map( array(
	'name' => __( 'RT Products', 'monsta' ),
	'base' => 'list_products_r',
	'icon' => 'icon-wpb-woocommerce',
	'category' => __( 'Theme', 'monsta' ),
	'description' => __( 'List all products on New', 'monsta' ),
	'params' => array_merge(
		array(
			array(
				'type' => 'textfield',
	...
	vc_map( array(
	'name' => __( 'RT Product categories', 'monsta' ),
	...
	'description' => __( 'List all products on New', 'monsta' ),
		'params' => array_merge( 
			array(
				array(
					'type' => 'textfield',
					'heading' => __( 'Number', 'monsta' ),
					'param_name' => 'number',
					'description' => __( 'The `number` field is used to display the number of products.', 'monsta' ),
				),
				...
				array(
					...
					'description' => __( 'Hide empty', 'monsta' ),
				),
				array(
					'type' => 'autocomplete',
					'heading' => __( 'Categories', 'monsta' ),
					'param_name' => 'ids',
					'settings' => array(
						'multiple' => true,
						'sortable' => true,
					...
					'save_always' => true,
					'description' => __( 'List of product categories', 'monsta' ),
				), 
			),road_get_slider_setting()
		)
	)  );
		...
		vc_map( array(
		'name' => __( 'RT Product category', 'monsta' ),
		'base' => 'product_category_r',
		'icon' => 'icon-wpb-woocommerce',
		'category' => __( 'Theme', 'monsta' ),
		'description' => __( 'Show multiple products in a category', 'monsta' ),
			'params' => array_merge(
			array(
				array(
					'type' => 'textfield',
